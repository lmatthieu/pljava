/*
 * Copyright (c) 2005, 2006 TADA AB - Taby Sweden Copyright (c) 2005, 2010, 2011
 * PostgreSQL Global Development Group
 * 
 * Distributed under the terms shown in the file COPYRIGHT found in the root
 * folder of this project or at
...
public class SPIDatabaseMetaData implements DatabaseMetaData
{
  ...
  private final SPIConnection m_connection;                                 // The
                                                                             // connection
                                                                             // association

  private static final int    VARHDRSZ       = 4;                           // length
                                                                             // for
                                                                             // int4

  private int                 NAMEDATALEN    = 0;                           // length
                                                                             // for
                                                                             // name
                                                                             // datatype

  private int                 INDEX_MAX_KEYS = 0;                           // maximum
                                                                             // number
                                                                             // of
                                                                             // keys
                                                                             // in
  ...
  {
    if ( INDEX_MAX_KEYS == 0 )
      INDEX_MAX_KEYS = Integer.parseInt( Backend
          .getConfigOption( "max_index_keys" ) );
   ...
   * user? @return true if so @exception SQLException if a database access error
   * occurs
   */
  ...
  /*
   * What is the URL for this database? @return the url or null if it cannott be
   * generated @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Are NULL values sorted high? @return true if so @exception SQLException if
  ...
  /*
   * Are NULL values sorted low? @return true if so @exception SQLException if a
  ...
  /*
   * Are NULL values sorted at the start regardless of sort order? @return true
  ...
  /*
   * Are NULL values sorted at the end regardless of sort order? @return true if
  ...
  /*
   * What is the name of this database product - we hope that it is PostgreSQL,
   * so we return that explicitly. @return the database product name @exception
   * SQLException if a database access error occurs
   */
  ...
  /*
   * What is the version of this database product. @return the database version
   * 
   ...
   * something wrong! @return the JDBC driver name @exception SQLException why?
   */
   ...
   * sensitive and as a result store them in mixed case? A JDBC-Compliant driver
   * will always return false. @return true if so @exception SQLException if a
   * database access error occurs
   */
   ...
   * and as a result store them in mixed case? A JDBC compliant driver will
   * always return true. @return true if so @exception SQLException if a
   * database access error occurs
   */
   ...
   * identifier quoting isn't supported. A JDBC Compliant driver will always use
   * a double quote character. @return the quoting string @exception
   * SQLException if a database access error occurs
   */
   ...
   * src/backend/parser/keywords.c <p>For SQL Keywords, I took the list provided
   * at <a href="http://web.dementia.org/~shadow/sql/sql3bnf.sep93.txt">
   * http://web.dementia.org/~shadow/sql/sql3bnf.sep93.txt</a> which is for
   * SQL3, not SQL-92, but it is close enough for this purpose. @return a comma
   * separated list of keywords we use @exception SQLException if a database
   * access error occurs
   */
  ...
  public String getTimeDateFunctions() throws SQLException
  {
    return BuiltinFunctions.CURDATE + ',' + BuiltinFunctions.CURTIME + ','
        ...
        + BuiltinFunctions.DAYOFWEEK + ',' + BuiltinFunctions.DAYOFYEAR + ','
        + BuiltinFunctions.HOUR + ',' + BuiltinFunctions.MINUTE + ','
        + BuiltinFunctions.MONTH + ',' + BuiltinFunctions.MONTHNAME + ','
        + BuiltinFunctions.NOW + ',' + BuiltinFunctions.QUARTER + ','
        + BuiltinFunctions.SECOND + ',' + BuiltinFunctions.WEEK + ','
   ...
   * src/backend/parser/scan.l, an identifier is {letter}{letter_or_digit} which
   * makes it just those listed above. @return a string containing the extra
   * characters @exception SQLException if a database access error occurs
   */
   ...
   * provide names for computed columns or to provide alias names for columns as
   * required. A JDBC Compliant driver always returns true. <p>e.g. <br><pre>
   ...
   * driver always returns true @return true if so @exception SQLException if a
   * database access error occurs
   */
   ...
   * different from the names of the tables? @return true if so; false otherwise
   * 
   * @exception SQLException - if a database access error occurs
   */
  ...
  /*
   * Can an "ORDER BY" clause use columns not in the SELECT? @return true if so
   * 
   * @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Can a "GROUP BY" clause add columns not in the SELECT provided it specifies
   * all the columns in the SELECT? Does anyone actually understand what they
   * mean here? (I think this is a subset of the previous function. -- petere)
   * 
   * @return true if so @exception SQLException if a database access error
   * occurs
   */
   ...
   * implemented it, but I dont think this is possible from the back ends point
   * of view. @return true if so @exception SQLException if a database access
   * error occurs
   */
   ...
   * I guess we can have, since Im relying on it. @return true if so @exception
   * SQLException if a database access error occurs
   */
   ...
   * returns true. <p>This changed from false to true in v6.2 of the driver, as
   * this support was added to the backend. @return true if so @exception
   * SQLException if a database access error occurs
   */
   ...
   * defined at: <p><a
   * href="http://www.microsoft.com/msdn/sdk/platforms/doc/odbc/src/intropr.htm"
   ...
   * minimal (Level 0) grammar. @return true if so @exception SQLException if a
   * database access error occurs
   */
   ...
   * conform to the Core (Level 1), so we can't conform to the Extended SQL
   * Grammar. @return true if so @exception SQLException if a database access
   * error occurs
   */
   ...
   * since we will be mostly compliant (probably more compliant than many other
   * databases) And since this is a requirement for all JDBC drivers we need to
   * get to the point where we can return true. @return true if so
   * 
   * @exception SQLException if a database access error occurs
   */
   ...
   * this means support for constraints @return true if so @exception
   * SQLException if a database access error occurs
   */
   ...
   * 
   * @return the vendor term @exception SQLException if a database access error
   * occurs
   */
  ...
  /*
   * Does a catalog appear at the start of a qualified table name? (Otherwise it
   * appears at the end). @return true if so @exception SQLException if a
   * database access error occurs
   */
  ...
  /*
   * Can a schema name be used in a data manipulation statement? @return true if
   * so @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Can a schema name be used in a procedure call statement? @return true if so
   * 
   * @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Can a schema name be used in an index definition statement? @return true if
   * so @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Can a schema name be used in a privilege definition statement? @return true
   * if so @exception SQLException if a database access error occurs
   */
  ...
  /*
   * Can a catalog name be used in a table definition statement? @return true if
   * so @exception SQLException if a database access error occurs
   */
   ...
   * positioned delete. @return true if so @exception SQLException if a database
   * access error occurs
   */
  ...
  /*
   * Is positioned UPDATE supported? @return true if so @exception SQLException
   * if a database access error occurs
   */
   ...
   * supported? @return true if so; false otherwise @exception SQLException - if
   * a database access error occurs
   */
   ...
   * always returns true. @return true if so; false otherwise
   * 
   * @exception SQLException - if a database access error occurs
   */
   ...
   * always returns true. (No idea what this is, but we support a good deal of
   * subquerying.) @return true if so; false otherwise @exception SQLException -
   * if a database access error occurs
   */
   ...
   * true. (a.k.a. subselect in from?) @return true if so; false otherwise
   * 
   * @exception SQLException - if a database access error occurs
   */
  ...
  /*
   * Is SQL UNION ALL supported? @return true if so @exception SQLException if a
   * database access error occurs
   */
   ...
   * guarentee that. In further reflection. we are talking a Statement object
   * here, so the answer is yes, since the Statement is only a vehicle to
   * ExecSQL() @return true if they always remain open; false otherwise
   * 
   * @exception SQLException if a database access error occurs
   */
   ...
   * cannot guarentee that. In further contemplation, we are talking a Statement
   * object here, so the answer is yes, since the Statement is only a vehicle to
   * ExecSQL() in Connection @return true if they always remain open; false
   ...
   * number of columns @exception SQLException if a database access error occurs
   */
   ...
   * data. A class can have no more than 1600 attributes (realistically, this is
   * limited by the fact that tuple sizes must be less than 8192 bytes)..."
   * 
   * @return the max columns @exception SQLException if a database access error
   * occurs
   */
   ...
   * accept() and fork(), its basically very high. Unless the system runs out of
   * processes, it can be 65535 (the number of aux. ports on a TCP/IP system). I
   * will return 8192 since that is what even the largest system can
   * realistically handle, @return the maximum number of connections
   * 
   * @exception SQLException if a database access error occurs
   */
   ...
   * that there is no limit or the limit is not known @exception SQLException if
   * a database access error occurs
   */
  ...
  /*
   * What is the maximum length of a procedure name @return the max name length
   * in bytes @exception SQLException if a database access error occurs
   */
  ...
  /*
   * What is the maximum length of a SQL statement? @return max length in bytes
   * 
   * @exception SQLException if a database access error occurs
   */
   ...
   * executed, we can have many. However, we can only really have one statement
   * per connection going at once (since they are executed serially) - so we
   * return one. @return the maximum @exception SQLException if a database
   * access error occurs
   */
  ...
  /*
   * What is the maximum length of a table name @return max name length in bytes
   * 
   * @exception SQLException if a database access error occurs
   */
   ...
   * to commit? I think this means something like: <p><pre> CREATE TABLE T (A
   * INT); INSERT INTO T (A) VALUES (2); BEGIN; UPDATE T SET A = A + 1; CREATE
   * TABLE X (A INT); SELECT A FROM T INTO X; COMMIT; </pre><p> does the CREATE
   * TABLE call cause a commit? The answer is no. @return true if so @exception
   * SQLException if a database access error occurs
   */
  ...
  /**
   * Creates a condition with the specified operator based on schema
   * specification:<BR>
   * <UL>
   ...
   * </UL>
   */
  private static String resolveSchemaConditionWithOperator( String expr,
      String schema, String operator )
  {
   ...
   * </UL>
   */
  private static String resolveSchemaPatternCondition( String expr,
      String schema )
  {
   ...
   * returned. They are ordered by PROCEDURE_SCHEM and PROCEDURE_NAME <p>Each
   * procedure description has the following columns: <ol>
   * <li><b>PROCEDURE_CAT</b> String => procedure catalog (may be null)
   * <li><b>PROCEDURE_SCHEM</b> String => procedure schema (may be null)
   * <li><b>PROCEDURE_NAME</b> String => procedure name <li><b>ResultSetField
   * 4</b> reserved (make it null) <li><b>ResultSetField 5</b> reserved (make it
   * null) <li><b>ResultSetField 6</b> reserved (make it null)
   * <li><b>REMARKS</b> String => explanatory comment on the procedure
   * <li><b>PROCEDURE_TYPE</b> short => kind of procedure <ul> <li>
   * procedureResultUnknown - May return a result <li> procedureNoResult - Does
   * not return a result <li> procedureReturnsResult - Returns a result </ul>
   * </ol> @param catalog - a catalog name; "" retrieves those without a
   * catalog; null means drop catalog name from criteria @param schemaParrern -
   * a schema name pattern; "" retrieves those without a schema - we ignore this
   * parameter @param procedureNamePattern - a procedure name pattern @return
   * ResultSet - each row is a procedure description @exception SQLException if
   * a database access error occurs
   */
      ...
      String schemaPattern, String procedureNamePattern ) throws SQLException
  {
    String sql = "SELECT NULL AS PROCEDURE_CAT, n.nspname AS PROCEDURE_SCHEM, p.proname AS PROCEDURE_NAME, NULL, NULL, NULL, d.description AS REMARKS, "
        ...
        + " AND "
        + resolveSchemaPatternCondition( "n.nspname", schemaPattern );
   ...
   * columns. <p>Only descriptions matching the schema, procedure and parameter
   * name criteria are returned. They are ordered by PROCEDURE_SCHEM and
   * PROCEDURE_NAME. Within this, the return value, if any, is first. Next are
   * the parameter descriptions in call order. The column descriptions follow in
   * column number order. <p>Each row in the ResultSet is a parameter
   * description or column description with the following fields: <ol>
   * <li><b>PROCEDURE_CAT</b> String => procedure catalog (may be null)
   ...
   * column/parameter: <ul><li>procedureColumnUnknown - nobody knows
   * <li>procedureColumnIn - IN parameter <li>procedureColumnInOut - INOUT
   * parameter <li>procedureColumnOut - OUT parameter <li>procedureColumnReturn
   * - procedure return value <li>procedureColumnResult - result column in
   * ResultSet </ul> <li><b>DATA_TYPE</b> short => SQL type from java.sql.Types
   * <li><b>TYPE_NAME</b> String => Data source specific type name
   * <li><b>PRECISION</b> int => precision <li><b>LENGTH</b> int => length in
   * bytes of data <li><b>SCALE</b> short => scale <li><b>RADIX</b> short =>
   * radix <li><b>NULLABLE</b> short => can it contain NULL?
   * <ul><li>procedureNoNulls - does not allow NULL values <li>procedureNullable
   * - allows NULL values <li>procedureNullableUnknown - nullability unknown
   * <li><b>REMARKS</b> String => comment describing parameter/column </ol>
   * 
   * @param catalog This is ignored in org.postgresql, advise this is set to
   * null @param schemaPattern @param procedureNamePattern a procedure name
   * pattern @param columnNamePattern a column name pattern, this is currently
   * ignored because postgresql does not name procedure parameters. @return each
   * row is a stored procedure parameter or column description @exception
   * SQLException if a database-access error occurs
   * 
      ...
      String columnNamePattern ) throws SQLException
  {
    ...
    ArrayList<Object[]> v = new ArrayList<Object[]>(); // The new ResultSet
                                                       // tuple stuff
    ...
    f[6] = new ResultSetField( "TYPE_NAME", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    f[12] = new ResultSetField( "REMARKS", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    while ( rs.next() )
    {
      ...
      if ( !returnTypeType.equals( "c" ) )
      {
        ...
        tuple[3] = "returnValue";
        tuple[4] = new Short(
            (short) java.sql.DatabaseMetaData.procedureColumnReturn );
        ...
        tuple[10] = null;
        tuple[11] = new Short(
            (short) java.sql.DatabaseMetaData.procedureNullableUnknown );
        tuple[12] = null;
        v.add( tuple );
      }
      ...
      for ( int i = 0; i < argTypes.length; i++ )
      {
        ...
        tuple[3] = "$" + ( i + 1 );
        tuple[4] = new Short(
            (short) java.sql.DatabaseMetaData.procedureColumnIn );
      ...
      if ( returnTypeType.equals( "c" ) )
      {
        ...
        while ( columnrs.next() )
        {
          ...
          tuple[4] = new Short(
              (short) java.sql.DatabaseMetaData.procedureColumnResult );
   ...
   * <p>Each table description has the following columns: <ol>
   * <li><b>TABLE_CAT</b> String => table catalog (may be null)
   * <li><b>TABLE_SCHEM</b> String => table schema (may be null)
   * <li><b>TABLE_NAME</b> String => table name <li><b>TABLE_TYPE</b> String =>
   * table type. Typical types are "TABLE", "VIEW", "SYSTEM TABLE",
   * "GLOBAL TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM".
   * <li><b>REMARKS</b> String => explanatory comment on the table </ol> <p>The
   * valid values for the types parameter are: "TABLE", "INDEX", "SEQUENCE",
   * "VIEW", "SYSTEM TABLE", "SYSTEM INDEX", "SYSTEM VIEW", "SYSTEM TOAST
   * TABLE", "SYSTEM TOAST INDEX", "TEMPORARY TABLE", and "TEMPORARY VIEW"
   * 
   * @param catalog a catalog name; For org.postgresql, this is ignored, and
   * should be set to null @param schemaPattern a schema name pattern @param
   * tableNamePattern a table name pattern. For all tables this should be "%"
   * 
   * @param types a list of table types to include; null returns all types
   * 
   * @return each row is a table description @exception SQLException if a
   * database-access error occurs.
   */
      ...
      String tableNamePattern, String types[] ) throws SQLException
  {
    ...
    {
      HashMap<String, String> clauses = s_tableTypeClauses.get( types[i] );
      if ( clauses != null )
      {
  ...
  private static final HashMap<String, HashMap<String, String>> s_tableTypeClauses;
  static
  {
    s_tableTypeClauses = new HashMap<String, HashMap<String, String>>();
    ...
    ht.put( "NOSCHEMAS",
        "c.relkind = 'v' AND c.relname LIKE 'pg!_%' ESCAPE '!'" );
    ht = new HashMap<String, String>();
  ...
  /*
   * Get the schema names available in this database. The results are ordered by
   * schema name. <P>The schema column is: <OL> <LI><B>TABLE_SCHEM</B> String =>
   * schema name </OL> @return ResultSet each row has a single String column
   ...
   * by catalog name. <P>The catalog column is: <OL> <LI><B>TABLE_CAT</B> String
   * => catalog name </OL> @return ResultSet each row has a single String column
  ...
  /*
   * Get the table types available in this database. The results are ordered by
   * table type. <P>The table type is: <OL> <LI><B>TABLE_TYPE</B> String =>
   * table type. Typical types are "TABLE", "VIEW", "SYSTEM TABLE", "GLOBAL
   * TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM". </OL> @return ResultSet
   * each row has a single String column that is a table type
   */
  public java.sql.ResultSet getTableTypes() throws SQLException
  {
    String types[] = s_tableTypeClauses.keySet().toArray(
        new String[s_tableTypeClauses.size()] );
  ...
  /*
   * Get a description of table columns available in a catalog. <P>Only column
   * descriptions matching the catalog, schema, table and column name criteria
   * are returned. They are ordered by TABLE_SCHEM, TABLE_NAME and
   * ORDINAL_POSITION. <P>Each column description has the following columns:
   * <OL> <LI><B>TABLE_CAT</B> String => table catalog (may be null)
   * <LI><B>TABLE_SCHEM</B> String => table schema (may be null)
   * <LI><B>TABLE_NAME</B> String => table name <LI><B>COLUMN_NAME</B> String =>
   * column name <LI><B>DATA_TYPE</B> short => SQL type from java.sql.Types
   ...
   * precision. <LI><B>BUFFER_LENGTH</B> is not used. <LI><B>DECIMAL_DIGITS</B>
   * int => the number of fractional digits <LI><B>NUM_PREC_RADIX</B> int =>
   * Radix (typically either 10 or 2) <LI><B>NULLABLE</B> int => is NULL
   * allowed? <UL> <LI> columnNoNulls - might not allow NULL values <LI>
   * columnNullable - definitely allows NULL values <LI> columnNullableUnknown -
   * nullability unknown </UL> <LI><B>REMARKS</B> String => comment describing
   * column (may be null) <LI><B>COLUMN_DEF</B> String => default value (may be
   * null) <LI><B>SQL_DATA_TYPE</B> int => unused <LI><B>SQL_DATETIME_SUB</B>
   * int => unused <LI><B>CHAR_OCTET_LENGTH</B> int => for char types the
   * maximum number of bytes in the column <LI><B>ORDINAL_POSITION</B> int =>
   * index of column in table (starting at 1) <LI><B>IS_NULLABLE</B> String =>
   * "NO" means column definitely does not allow NULL values; "YES" means the
   * column might allow NULL values. An empty string means nobody knows. </OL>
   * 
   * @param catalog a catalog name; "" retrieves those without a catalog @param
   * schemaPattern a schema name pattern; "" retrieves those without a schema
   * 
   * @param tableNamePattern a table name pattern @param columnNamePattern a
   * column name pattern @return ResultSet each row is a column description
   * 
   * @see #getSearchStringEscape
   */
      ...
      String tableNamePattern, String columnNamePattern ) throws SQLException
  {
    ...
    f[0] = new ResultSetField( "TABLE_CAT", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    f[5] = new ResultSetField( "TYPE_NAME", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    f[11] = new ResultSetField( "REMARKS", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    while ( rs.next() )
    {
      ...
      if ( pgType.equals( "bpchar" ) || pgType.equals( "varchar" ) )
      {
        ...
        tuple[6] = new Integer( atttypmod != -1 ? atttypmod - VARHDRSZ : 0 );
      }
   ...
   * privileges matching the column name criteria are returned. They are ordered
   * by COLUMN_NAME and PRIVILEGE. <P>Each privilige description has the
   * following columns: <OL> <LI><B>TABLE_CAT</B> String => table catalog (may
   * be null) <LI><B>TABLE_SCHEM</B> String => table schema (may be null)
   * <LI><B>TABLE_NAME</B> String => table name <LI><B>COLUMN_NAME</B> String =>
   * column name <LI><B>GRANTOR</B> => grantor of access (may be null)
   * <LI><B>GRANTEE</B> String => grantee of access <LI><B>PRIVILEGE</B> String
   * => name of access (SELECT, INSERT, UPDATE, REFRENCES, ...)
   * <LI><B>IS_GRANTABLE</B> String => "YES" if grantee is permitted to grant to
   * others; "NO" if not; null if unknown </OL> @param catalog a catalog name;
   * "" retrieves those without a catalog @param schema a schema name; ""
   * retrieves those without a schema @param table a table name @param
      ...
      String table, String columnNamePattern ) throws SQLException
  {
    ...
    f[3] = new ResultSetField( "COLUMN_NAME", TypeOid.VARCHAR,
        getMaxNameLength() );
    f[4] = new ResultSetField( "GRANTOR", TypeOid.VARCHAR, getMaxNameLength() );
    f[5] = new ResultSetField( "GRANTEE", TypeOid.VARCHAR, getMaxNameLength() );
    f[6] = new ResultSetField( "PRIVILEGE", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    while ( rs.next() )
    {
      ...
      acls = (String[]) rs.getObject( "relacl" ); // TODO check if can change to
                                                  // getString
      ...
      {
        ArrayList<String> grantees = (ArrayList<String>) permissions
            .get( permNames[i] );
        for ( int j = 0; j < grantees.size(); j++ )
        {
          String grantee = grantees.get( j );
   ...
   * matching the schema and table name criteria are returned. They are ordered
   * by TABLE_SCHEM, TABLE_NAME, and PRIVILEGE. <P>Each privilige description
   * has the following columns: <OL> <LI><B>TABLE_CAT</B> String => table
   * catalog (may be null) <LI><B>TABLE_SCHEM</B> String => table schema (may be
   * null) <LI><B>TABLE_NAME</B> String => table name <LI><B>GRANTOR</B> =>
   ...
   * UPDATE, REFRENCES, ...) <LI><B>IS_GRANTABLE</B> String => "YES" if grantee
   * is permitted to grant to others; "NO" if not; null if unknown </OL> @param
   ...
   * schema @param tableNamePattern a table name pattern @return ResultSet each
   * row is a table privilege description
   * 
   * @see #getSearchStringEscape
   */
      ...
      String schemaPattern, String tableNamePattern ) throws SQLException
  {
    ...
    f[2] = new ResultSetField( "TABLE_NAME", TypeOid.VARCHAR,
        getMaxNameLength() );
    f[3] = new ResultSetField( "GRANTOR", TypeOid.VARCHAR, getMaxNameLength() );
    f[4] = new ResultSetField( "GRANTEE", TypeOid.VARCHAR, getMaxNameLength() );
    f[5] = new ResultSetField( "PRIVILEGE", TypeOid.VARCHAR, getMaxNameLength() );
   ...
   */
  private void addACLPrivileges( String acl,
      HashMap<String, ArrayList<String>> privileges )
  {
    ...
    for ( int i = 0; i < privs.length(); i++ )
    {
      ...
      ArrayList<String> usersWithPermission = (ArrayList<String>) privileges
          .get( sqlpriv );
   ...
   * permission.
   */
  protected HashMap<String, ArrayList<String>> parseACL( String[] aclArray,
      String owner )
  {
   ...
   * identifies a row. They are ordered by SCOPE. <P>Each column description has
   * the following columns: <OL> <LI><B>SCOPE</B> short => actual scope of
   * result <UL> <LI> bestRowTemporary - very temporary, while using row <LI>
   * bestRowTransaction - valid for remainder of current transaction <LI>
   * bestRowSession - valid for remainder of current session </UL>
   * <LI><B>COLUMN_NAME</B> String => column name <LI><B>DATA_TYPE</B> short =>
   * SQL data type from java.sql.Types <LI><B>TYPE_NAME</B> String => Data
   * source dependent type name <LI><B>COLUMN_SIZE</B> int => precision
   * <LI><B>BUFFER_LENGTH</B> int => not used <LI><B>DECIMAL_DIGITS</B> short =>
   * scale <LI><B>PSEUDO_COLUMN</B> short => is this a pseudo column like an
   * Oracle ROWID <UL> <LI> bestRowUnknown - may or may not be pseudo column
   * <LI> bestRowNotPseudo - is NOT a pseudo column <LI> bestRowPseudo - is a
   * pseudo column </UL> </OL> @param catalog a catalog name; "" retrieves those
   * without a catalog
   * 
   ...
   * SCOPE @param nullable include columns that are nullable? @return ResultSet
      ...
      throws SQLException
  {
    ...
    f[3] = new ResultSetField( "TYPE_NAME", TypeOid.VARCHAR, getMaxNameLength() );
    ...
    /*
     * At the moment this simply returns a table's primary key, if there is one.
     * I believe other unique indexes, ctid, and oid should also be considered.
     * -KJ
     */
    ...
    String sql = "SELECT a.attname, a.atttypid as atttypid " + from
        ...
        + " AND a.attrelid=ci.oid AND i.indisprimary " + " AND ct.relname = '"
        + escapeQuotes( table ) + "' " + where + " ORDER BY a.attnum ";
  ...
  /*
   * Get a description of a table's columns that are automatically updated when
   * any value in a row is updated. They are unordered. <P>Each column
   * description has the following columns: <OL> <LI><B>SCOPE</B> short => is
   * not used <LI><B>COLUMN_NAME</B> String => column name <LI><B>DATA_TYPE</B>
   ...
   * <LI><B>DECIMAL_DIGITS</B> short => scale <LI><B>PSEUDO_COLUMN</B> short =>
   * is this a pseudo column like an Oracle ROWID <UL> <LI> versionColumnUnknown
   * - may or may not be pseudo column <LI> versionColumnNotPseudo - is NOT a
   * pseudo column <LI> versionColumnPseudo - is a pseudo column </UL> </OL>
   * 
   * @param catalog a catalog name; "" retrieves those without a catalog @param
   * schema a schema name; "" retrieves those without a schema @param table a
   * table name @return ResultSet each row is a column description
   */
      ...
      String table ) throws SQLException
  {
    ...
    /*
     * Postgresql does not have any column types that are automatically updated
     * like some databases' timestamp type. We can't tell what rules or triggers
     * might be doing, so we are left with the system columns that change on an
     * update. An update may change all of the following system columns: ctid,
     * xmax, xmin, cmax, and cmin. Depending on if we are in a transaction and
     * wether we roll it back or not the only guaranteed change is to ctid. -KJ
     */
  ...
  /**
   * Added to support Java 7 DatabaseMetaData interface TODO This will need an
   * update to inquiry Postgresql catalog using TABLE_CAT,TABLE_SCHEM,
   * TABLE_NAME and COLUMN_NAME and returning the following columns TABLE_CAT
   * String => table catalog (may be null) TABLE_SCHEM String => table schema
   * (may be null) TABLE_NAME String => table name COLUMN_NAME String => column
   * name DATA_TYPE int => SQL type from java.sql.Types COLUMN_SIZE int =>
   ...
   * => The allowed usage for the column. The value returned will correspond to
   * the enum name returned by PseudoColumnUsage.name() REMARKS String =>
   * comment describing column (may be null) CHAR_OCTET_LENGTH int => for char
   * types the maximum number of bytes in the column IS_NULLABLE String => ISO
   * rules are used to determine the nullability for a column. YES --- if the
   * column can include NULLs NO --- if the column cannot include NULLs empty
   ...
   * <LI><B>TABLE_SCHEM</B> String => table schema (may be null)
   * <LI><B>TABLE_NAME</B> String => table name <LI><B>COLUMN_NAME</B> String =>
   * column name <LI><B>KEY_SEQ</B> short => sequence number within primary key
   * <LI><B>PK_NAME</B> String => primary key name (may be null) </OL> @param
   * catalog a catalog name; "" retrieves those without a catalog @param schema
   * a schema name pattern; "" retrieves those without a schema @param table a
   * table name @return ResultSet each row is a primary key column description
   */
   ...
   * @param primarySchema
   * @param primaryTable
   *          if provided will get the keys exported by this table
   * @param foreignTable
      ...
      String foreignSchema, String foreignTable ) throws SQLException
  {
    ...
    f[11] = new ResultSetField( "FK_NAME", TypeOid.VARCHAR, getMaxNameLength() );
    f[12] = new ResultSetField( "PK_NAME", TypeOid.VARCHAR, getMaxNameLength() );
    f[13] = new ResultSetField( "DEFERRABILITY", TypeOid.INT2, 2 );

    /*
     * The addition of the pg_constraint in 7.3 table should have really helped
     * us out here, but it comes up just a bit short. - The conkey, confkey
     * columns aren't really useful without contrib/array unless we want to
     * issues separate queries. - Unique indexes that can support foreign keys
     * are not necessarily added to pg_constraint. Also multiple unique indexes
     * covering the same keys can be created which make it difficult to
     * determine the PK_NAME field.
     */
    String sql = "SELECT NULL::text AS PKTABLE_CAT, pkn.nspname AS PKTABLE_SCHEM, pkc.relname AS PKTABLE_NAME, pka.attname AS PKCOLUMN_NAME, "
        ...
        + " AND "
        + resolveSchemaCondition( "pkn.nspname", primarySchema )
   ...
   * (may be null) <LI><B>PKTABLE_SCHEM</B> String => primary key table schema
   * being imported (may be null) <LI><B>PKTABLE_NAME</B> String => primary key
   * table name being imported <LI><B>PKCOLUMN_NAME</B> String => primary key
   * column name being imported <LI><B>FKTABLE_CAT</B> String => foreign key
   * table catalog (may be null) <LI><B>FKTABLE_SCHEM</B> String => foreign key
   * table schema (may be null) <LI><B>FKTABLE_NAME</B> String => foreign key
   * table name <LI><B>FKCOLUMN_NAME</B> String => foreign key column name
   * <LI><B>KEY_SEQ</B> short => sequence number within foreign key
   * <LI><B>UPDATE_RULE</B> short => What happens to foreign key when primary is
   * updated: <UL> <LI> importedKeyCascade - change imported key to agree with
   * primary key update <LI> importedKeyRestrict - do not allow update of
   * primary key if it has been imported <LI> importedKeySetNull - change
   * imported key to NULL if its primary key has been updated </UL>
   * <LI><B>DELETE_RULE</B> short => What happens to the foreign key when
   * primary is deleted. <UL> <LI> importedKeyCascade - delete rows that import
   * a deleted key <LI> importedKeyRestrict - do not allow delete of primary key
   * if it has been imported <LI> importedKeySetNull - change imported key to
   * NULL if its primary key has been deleted </UL> <LI><B>FK_NAME</B> String =>
   * foreign key name (may be null) <LI><B>PK_NAME</B> String => primary key
   * name (may be null) </OL> @param catalog a catalog name; "" retrieves those
   * without a catalog @param schema a schema name pattern; "" retrieves those
   * without a schema @param table a table name @return ResultSet each row is a
  ...
  /*
   * Get a description of a foreign key columns that reference a table's primary
   * key columns (the foreign keys exported by a table). They are ordered by
   * FKTABLE_CAT, FKTABLE_SCHEM, FKTABLE_NAME, and KEY_SEQ. This method is
   * currently unimplemented. <P>Each foreign key column description has the
   * following columns: <OL> <LI><B>PKTABLE_CAT</B> String => primary key table
   * catalog (may be null) <LI><B>PKTABLE_SCHEM</B> String => primary key table
   * schema (may be null) <LI><B>PKTABLE_NAME</B> String => primary key table
   * name <LI><B>PKCOLUMN_NAME</B> String => primary key column name
   * <LI><B>FKTABLE_CAT</B> String => foreign key table catalog (may be null)
   * being exported (may be null) <LI><B>FKTABLE_SCHEM</B> String => foreign key
   * table schema (may be null) being exported (may be null)
   ...
   * if it has been imported <LI> importedKeySetNull - change imported key to
   * NULL if its primary key has been deleted </UL> <LI><B>FK_NAME</B> String =>
   * foreign key identifier (may be null) <LI><B>PK_NAME</B> String => primary
   * key identifier (may be null) </OL> @param catalog a catalog name; ""
  ...
  /*
   * Get a description of the foreign key columns in the foreign key table that
   * reference the primary key columns of the primary key table (describe how
   * one table imports another's key.) This should normally return a single
   ...
   * and KEY_SEQ. This method is currently unimplemented. <P>Each foreign key
   * column description has the following columns: <OL> <LI><B>PKTABLE_CAT</B>
   ...
   * String => primary key table name <LI><B>PKCOLUMN_NAME</B> String => primary
   * key column name <LI><B>FKTABLE_CAT</B> String => foreign key table catalog
   * (may be null) being exported (may be null) <LI><B>FKTABLE_SCHEM</B> String
   ...
   * the corresponding JDBC SQL type. <P>Each type description has the following
   * columns: <OL> <LI><B>TYPE_NAME</B> String => Type name <LI><B>DATA_TYPE</B>
   * short => SQL data type from java.sql.Types <LI><B>PRECISION</B> int =>
   * maximum precision <LI><B>LITERAL_PREFIX</B> String => prefix used to quote
   * a literal (may be null) <LI><B>LITERAL_SUFFIX</B> String => suffix used to
   * quote a literal (may be null) <LI><B>CREATE_PARAMS</B> String => parameters
   * used in creating the type (may be null) <LI><B>NULLABLE</B> short => can
   * you use NULL for this type? <UL> <LI> typeNoNulls - does not allow NULL
   * values <LI> typeNullable - allows NULL values <LI> typeNullableUnknown -
   * nullability unknown </UL> <LI><B>CASE_SENSITIVE</B> boolean=> is it case
   * sensitive? <LI><B>SEARCHABLE</B> short => can you use "WHERE" based on this
   * type: <UL> <LI> typePredNone - No support <LI> typePredChar - Only
   * supported with WHERE .. LIKE <LI> typePredBasic - Supported except for
   * WHERE .. LIKE <LI> typeSearchable - Supported for all WHERE .. </UL>
   * <LI><B>UNSIGNED_ATTRIBUTE</B> boolean => is it unsigned?
   * <LI><B>FIXED_PREC_SCALE</B> boolean => can it be a money value?
   * <LI><B>AUTO_INCREMENT</B> boolean => can it be used for an auto-increment
   * value? <LI><B>LOCAL_TYPE_NAME</B> String => localized version of type name
   * (may be null) <LI><B>MINIMUM_SCALE</B> short => minimum scale supported
   * <LI><B>MAXIMUM_SCALE</B> short => maximum scale supported
   * <LI><B>SQL_DATA_TYPE</B> int => unused <LI><B>SQL_DATETIME_SUB</B> int =>
   * unused <LI><B>NUM_PREC_RADIX</B> int => usually 2 or 10 </OL>
   * 
  ...
  public java.sql.ResultSet getTypeInfo() throws SQLException
  {
    ...
    f[0] = new ResultSetField( "TYPE_NAME", TypeOid.VARCHAR, getMaxNameLength() );
   ...
   * NON_UNIQUE, TYPE, INDEX_NAME, and ORDINAL_POSITION. <P>Each index column
   * description has the following columns: <OL> <LI><B>TABLE_CAT</B> String =>
   * table catalog (may be null) <LI><B>TABLE_SCHEM</B> String => table schema
   * (may be null) <LI><B>TABLE_NAME</B> String => table name
   * <LI><B>NON_UNIQUE</B> boolean => Can index values be non-unique? false when
   * TYPE is tableIndexStatistic <LI><B>INDEX_QUALIFIER</B> String => index
   * catalog (may be null); null when TYPE is tableIndexStatistic
   * <LI><B>INDEX_NAME</B> String => index name; null when TYPE is
   * tableIndexStatistic <LI><B>TYPE</B> short => index type: <UL> <LI>
   * tableIndexStatistic - this identifies table statistics that are returned in
   * conjuction with a table's index descriptions <LI> tableIndexClustered -
   * this is a clustered index <LI> tableIndexHashed - this is a hashed index
   * <LI> tableIndexOther - this is some other style of index </UL>
   * <LI><B>ORDINAL_POSITION</B> short => column sequence number within index;
   * zero when TYPE is tableIndexStatistic <LI><B>COLUMN_NAME</B> String =>
   * column name; null when TYPE is tableIndexStatistic <LI><B>ASC_OR_DESC</B>
   * String => column sort sequence, "A" => ascending "D" => descending, may be
   * null if sort sequence is not supported; null when TYPE is
   * tableIndexStatistic <LI><B>CARDINALITY</B> int => When TYPE is
   * tableIndexStatisic then this is the number of rows in the table; otherwise
   * it is the number of unique values in the index. <LI><B>PAGES</B> int =>
   ...
   * null) </OL> @param catalog a catalog name; "" retrieves those without a
   * catalog @param schema a schema name pattern; "" retrieves those without a
   * schema @param table a table name @param unique when true, return only
   * indices for unique values; when false, return indices regardless of whether
   * unique or not @param approximate when true, result is allowed to reflect
   * approximate or out of data values; when false, results are requested to be
   * accurate @return ResultSet each row is an index column description
   */
  // Implementation note: This is required for Borland's JBuilder to work
      ...
      throws SQLException
  {
    ...
    String where = " AND n.oid = ct.relnamespace " + " AND "
        + resolveSchemaCondition( "n.nspname", schema );
  ...
  /*
   * Does the database support the given result set type? @param type - defined
   * in java.sql.ResultSet @return true if so; false otherwise
   * 
   * @exception SQLException - if a database access error occurs
   */
   ...
   * concurrency - type defined in java.sql.ResultSet @return true if so; false
   * otherwise @exception SQLException - if a database access error occurs
   */
    ...
    if ( typeNamePattern != null )
    {
      ...
      // be
      {
        if ( firstQualifier != secondQualifier )
        {
          ...
          schemaPattern = typeNamePattern.substring( firstQualifier + 1,
              secondQualifier );
        }
        else
        {
   ...
   * in a particular schema in this database. Only the immediate super type/ sub
   * type relationship is modeled.
   * <P>
   * Only supertype information for UDTs matching the catalog, schema, and type
   * name is returned. The type name parameter may be a fully-qualified name.
   * When the UDT name supplied is a fully-qualified name, the catalog and
   ...
   * table name are returned. The table name parameter may be a fully- qualified
   * name, in which case, the catalog and schemaPattern parameters are ignored.
   * If a table does not have a super table, it is not listed here. Supertables
   * have to be defined in the same catalog and schema as the sub tables.
   * Therefore, the type description does not need to include this information
   * for the supertable.
   * <P>
  ...
  /**
   * Added to support Java 7 DatabaseMetaData interface
   */
   ...
   *          narrow the search
   * @param typeNamePattern
   *          a type name pattern; must match the type name as it is stored in
   *          the database
  ...
  public boolean locatorsUpdateCopy() throws SQLException
  {
     ...
     * return. We don't throw the notImplemented Exception because the 1.5 JDK's
     * CachedRowSet calls this method regardless of wether large objects are
     * used.
     */
    return true;
  }
  ...
  /**
   * This method creates a ResultSet which is not associated with any statement.
   */
  private ResultSet createSyntheticResultSet( ResultSetField[] f,
      ArrayList<Object[]> tuples ) throws SQLException
  {
  ...
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.getFunctions( String, String, String ) not implemented yet.",
        "0A000" );
  ...
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.getClientInfoProperties() not implemented yet.",
        "0A000" );
  ...
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.autoCommitFailureClosesAllResultSets() not implemented yet.",
        "0A000" );
  ...
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.supportsStoredFunctionsUsingCallSyntax() not implemented yet.",
        "0A000" );
      ...
      throws SQLException
  {
    ...
    "SPIDatabaseMetadata.getSchemas( String, String ) not implemented yet.",
        "0A000" );
  ...
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.isWrapperFor( Class< ? > ) not implemented yet.",
        "0A000" );

  }

  public <T> T unwrap( java.lang.Class<T> T ) throws SQLException
  {
    throw new SQLFeatureNotSupportedException(
        "SPIDatabaseMetadata.unwrap( Class< T > ) not implemented yet.",
        "0A000" );
